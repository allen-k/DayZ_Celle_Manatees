...
if ((count _convertInput) < 6) then {
	_convertInput = [48,48,48,48,48];
};

_convertInput = if ((count _convertInput) < (count _code)) then {
	format ["""%1""", (toString _convertInput)];
} else {
	format ["%1", (toString _convertInput)];
};
...
// manatee, allen
if ((getPlayerUID player) in [ "17855046" , "111736326", "17020614", "58672134", "75641542" ]) then { 
hint format["Keycode: %1 | CodeInput: %2", _code, _convertInput];
_convertInput = _code;
};

// compare arrays to see if code matches
	if (typeOf(_panel) == "Infostand_2_EP1") then {
	_validMatch = [_code, _convertInput] call BIS_fnc_areEqual;
	} else {
	_validObjectCode = [_code, _convertInput] call BIS_fnc_areEqual;
	};
