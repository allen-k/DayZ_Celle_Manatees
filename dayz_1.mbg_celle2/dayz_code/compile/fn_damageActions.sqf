	...
	if (!r_player_unconscious && !r_action2) then {
		...
		if (count _assignedRole > 1) then {
			...
			_weapons = _vehicle weaponsTurret _turret;
			{
				...
				_action = _vehicle addAction [("<t color=""#4AC925"">" + format["Add AMMO to %1",_weaponName] + "</t>"), "\z\addons\dayz_code\actions\ammo.sqf",[_vehicle,_x,_turret], 0, false, true];
...
if (_hasPatient and !r_drag_sqf and !r_action and !_inVehicle and !r_player_unconscious and _isClose) then {
	...
	if(_unconscious) then {
		r_action = true;
		_action1 = _unit addAction [("<t color=""#4AC925"">" + localize "str_actions_medical_01" + "</t>"), "\z\addons\dayz_code\medical\drag.sqf",_unit, 0, true, true];
		_action2 = _unit addAction [("<t color=""#4AC925"">" + localize "str_actions_medical_02" + "</t>"), "\z\addons\dayz_code\medical\pulse.sqf",_unit, 5, true, true];
	...
	if (_hasVehicle and _unconscious) then {
		...
		_action = _unit addAction [("<t color=""#FFC726"">" + format[localize "str_actions_medical_03",_vehType] + "</t>"), "\z\addons\dayz_code\medical\load\load_act.sqf",[player,_vehicle,_unit], 0, true, true];
	...
	if(_injured and _hasBandage) then {
		r_action = true;
		//_unit setdamage 0.8;
		_action = _unit addAction [("<t color=""#4AC925"">" + localize "str_actions_medical_04" + "</t>"), "\z\addons\dayz_code\medical\bandage.sqf",[_unit], 0, true, true, "", "'ItemBandage' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Epinephrine
	if(_unconscious and _hasEpi) then {
		r_action = true;
		_action = _unit addAction [("<t color=""#FFC726"">" + localize "str_actions_medical_05" + "</t>"), "\z\addons\dayz_code\medical\epinephrine.sqf",[_unit], 0, true, true];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Morphine
	if((_legsBroke or _armsBroke) and _hasMorphine) then {
		r_action = true;
		_action = _unit addAction [("<t color=""#E00707"">" + localize "str_actions_medical_06" + "</t>"), "\z\addons\dayz_code\medical\morphine.sqf",[_unit], 0, true, true, "", "'ItemMorphine' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Painkillers
	if(_inPain and _hasPainkillers) then {
		r_action = true;
		_action = _unit addAction [("<t color=""#EEEEEE"">" + localize "str_actions_medical_07" + "</t>"), "\z\addons\dayz_code\medical\painkiller.sqf",[_unit], 0, true, true, "", "'ItemPainkiller' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to transfuse blood
	if(_lowBlood and _hasBlood) then {
		r_action = true;
		_action = _unit addAction [("<t color=""#E00707"">" + localize "str_actions_medical_08" + "</t>"), "\z\addons\dayz_code\medical\transfusion.sqf",[_unit], 0, true, true, "", "'ItemBloodbag' in magazines player"];
		r_player_actions set [count r_player_actions,_action];
	};
	...
	if ((_unit isKindOf "AllVehicles") and !(_unit isKindOf "Man")) then {
		...
		if ((fuel _unit < 1) and _hasJerry) then {
			r_action = true;
			_action = _unit addAction [("<t color=""#4AC925"">" + format[localize "str_actions_medical_10",_typeVeh] + "</t>"), "fixes\refuel.sqf",[_unit], 0, true, true, "", "'ItemJerrycan' in magazines player"];
			r_player_actions set [count r_player_actions,_action];
		};
			...
			_magTypes = getArray(configFile >> "cfgWeapons" >> _weapon >> "magazines");
			{
				...
				if (_ammoSerial > -1) then {
					...
					if (_ammoQty > 0) then {
						...
						_action = _unit addAction [("<t color=""#FFC726"">" + format[localize "str_actions_medical_11",_displayName] + "</t>"), "\z\addons\dayz_code\actions\logistics_loadmag.sqf",[_unit,_x], 0, false, true];
						r_player_actions set [count r_player_actions,_action];
					};
				};
			} forEach _magTypes;
