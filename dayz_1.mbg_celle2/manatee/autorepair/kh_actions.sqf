    ...
    if (!isNull player) then {
        ...
        if (_vehicle != _currentVehicle) then {
            if (!isNull _vehicle) then {
                _vehicle removeAction vehicle_refuel_id;
                _vehicle = objNull;
            };
            ...
            if (_currentVehicle != player && _isNearFeed && !(_currentVehicle isKindof "Bicycle")) then {
                ...
                vehicle_refuel_id = _vehicle addAction [("<t color=""#FFC726"">" + ("Auto-Refuel") +"</t>"), "manatee\autorepair\kh_vehicle_refuel.sqf", [_amount], 10, false, true, "", "vehicle _this == _target && local _target"];
            };
        };
