...
_totalSleepTime = 180; // Total ammount of time it takes before you wake up

_bloodAmount = 12000; // Total amount of blood to give back to player on sleep (12000 is the max life for a player)

_sleepCooldown = 7200; // This is how long in seconds before you can sleep again
...
if (dayz_combat == 1) then { // Check if in combat (dunno why you would want to sleep in combat but the check is here anyway)
	...
	if (_timeLeft == 0 || _timeLeft < 0) then { // If sleep countdown is done then.....
		...
		cutText [format["You are awake and feel like shit!"], "PLAIN DOWN"]; // Text displayed on wakeup!
